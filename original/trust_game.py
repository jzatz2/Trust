...
class TrustManager:
    ...
    def process_ai_avoidance(self, avoided_successfully, obstacle_distance):
        """
        Process AI avoidance action and resolve user interventions
        """
        current_time = self.get_elapsed_time()
        
        # Resolve recent interventions (within last 3 seconds)
        for intervention in self.pending_interventions:
            if not intervention['resolved'] and (current_time - intervention['time']) < 3.0:
                # User signaled they would intervene (lack of trust in AI)
                # If AI avoided successfully, user's intervention was unnecessary
                # If AI crashed, user's instinct to intervene was correct
                
                user_intervention_justified = not avoided_successfully
                
                # Update trust based on intervention outcome
                # humanResponse = True means user wanted to intervene (pressed button)
                # robotPerformance = whether AI succeeded

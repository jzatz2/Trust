...
EDGE_CASE_CONFIG = {
    'enable_forced_crash': False,  # Set to True to enable inevitable crash every 10s
    'enable_ai_freeze': True,      # Set to True to enable AI freeze every 10s
    'crash_interval': 10.0,         # Seconds between forced crashes
    'freeze_interval': 10.0,        # Seconds between AI freezes
    'freeze_duration': 10.0         # Duration of each freeze in seconds
}
...
class AICar:
    ...
    def make_avoidance_decision(self, obstacles, current_time):
        ...
        """
        # Edge case: If AI is frozen, don't make decisions
        if self.is_frozen:
...
class GameEnvironment:
    ...
    def __init__(self, config, edge_case_config):
        self.config = config
        ...
        # Edge case tracking
        self.last_forced_crash_time = 0
        self.last_freeze_toggle_time = 0
    ...
    def spawn_forced_crash_traffic(self):
        """Edge case: Spawn traffic in all lanes (inevitable crash)"""
        for lane in range(self.config['num_lanes']):
            x = self.config['road_x_offset'] + (lane * self.config['lane_width']) + (self.config['lane_width'] // 2)
            speed = np.random.uniform(self.config['traffic_speed_min'], self.config['traffic_speed_max'])
        ...
        print("⚠️  EDGE CASE: Forced crash - obstacles spawned in all lanes!")
    ...
    def handle_edge_cases(self, current_time, ai_car):
        """Handle edge case logic"""
        # Edge case 1: Forced crash every 10 seconds
        if self.edge_case_config['enable_forced_crash']:
            if current_time - self.last_forced_crash_time >= self.edge_case_config['crash_interval']:
                self.spawn_forced_crash_traffic()
                self.last_forced_crash_time = current_time
        
        # Edge case 2: AI freeze every 10 seconds (10s frozen, 10s normal)
        if self.edge_case_config['enable_ai_freeze']:
            time_in_cycle = current_time - self.last_freeze_toggle_time
            freeze_interval = self.edge_case_config['freeze_interval']
            freeze_duration = self.edge_case_config['freeze_duration']
            
            # Cycle: 0-10s normal, 10-20s frozen, 20-30s normal, 30-40s frozen, etc.
            if time_in_cycle >= freeze_interval + freeze_duration:
                # Reset cycle
                self.last_freeze_toggle_time = current_time
                ai_car.is_frozen = False
            elif time_in_cycle >= freeze_interval and not ai_car.is_frozen:
                # Start freeze
                ai_car.is_frozen = True
                print("❄️  EDGE CASE: AI FROZEN - no avoidance for 10 seconds!")
            elif time_in_cycle < freeze_interval and ai_car.is_frozen:
                # End freeze
                ai_car.is_frozen = False
                print("✅ EDGE CASE: AI UNFROZEN - normal behavior resumed")
    ...
    def render(self, ai_car, trust_level, user_input_state, elapsed_time):
        ...
        car_pygame_rect = pygame.Rect(car_rect['x'], car_rect['y'], car_rect['width'], car_rect['height'])
        car_color = (100, 100, 255) if ai_car.is_frozen else colors['ai_car']  # Lighter blue when frozen
        pygame.draw.rect(self.screen, car_color, car_pygame_rect)
        ...
        # AI Freeze indicator
        if ai_car.is_frozen:
            freeze_text = 'AI FROZEN (EDGE CASE)'
            freeze_surface = self.small_font.render(freeze_text, True, (255, 255, 0))
            freeze_rect = freeze_surface.get_rect(center=(500, 150))
            pygame.draw.rect(self.screen, (50, 50, 150), freeze_rect.inflate(20, 10))
...
class TrustManager:
    ...
    def process_ai_avoidance(self, avoided_successfully, obstacle_distance):
        ...
        # Check if there are any recent user interventions
        has_recent_intervention = any(
            not i['resolved'] and (current_time - i['time']) < 3.0 
            for i in self.pending_interventions
        )
        ...
        for intervention in self.pending_interventions:
            if not intervention['resolved'] and (current_time - intervention['time']) < 3.0:
                ...
                print(f"✓ Intervention resolved: User intervention was {'JUSTIFIED' if user_intervention_justified else 'UNNECESSARY'} | "
                      f"AI {'AVOIDED' if avoided_successfully else 'CRASHED'} | "
                      f"Trust: {self.get_trust_level():.3f}")
        
        # UPDATE trust for autonomous actions (when no user intervention)
        if not has_recent_intervention:
            context = {
                'speed': 4.0,
                'ttc': obstacle_distance / 50.0 if obstacle_distance else 1.0
            }
            ...
            trust_result = self.trust_model.updateTrust(
                robotAction='avoid_obstacle',
                robotPerformance=avoided_successfully,
                humanResponse=False,  # No user intervention = implicit trust/acceptance
                context=context
            )
            
            if avoided_successfully:
                print(f"🤖 AI avoided autonomously (no user input) | Trust: {self.get_trust_level():.3f}")
...
# ==================== MAIN GAME LOOP ====================
def run_game(config, edge_case_config):
    ai_car = AICar(config)
    environment = GameEnvironment(config, edge_case_config)
    ...
    if edge_case_config['enable_forced_crash']:
        print("⚠️  EDGE CASE ENABLED: Forced crashes every 10 seconds")
    if edge_case_config['enable_ai_freeze']:
        print("❄️  EDGE CASE ENABLED: AI freeze cycles (10s normal, 10s frozen)")
    print()
    ...
    while running:
        ...
        # Handle edge cases (forced crashes, AI freezes)
        environment.handle_edge_cases(current_time, ai_car)
...
if __name__ == "__main__":
    ...
    try:
        pygame.init()
        run_game(GAME_CONFIG, EDGE_CASE_CONFIG)
